...
const KAME_AUDIO_BLAST_START_MS = 4000; // Bắn ở giây thứ 4
...
let isShaking = false;
let shakeTimer = 0;
let isShowingInstructions = true;
...
// (Toàn bộ các Class: Particle, createExplosion, ChargingEffect, KamehamehaEffect, Player, Projectile, Obstacle, ObstacleType2, EnemyProjectile, Boss... giữ nguyên)
// ... (Bạn không cần thay đổi bất cứ class nào) ...
...
class Obstacle {
  constructor() {
    ...
    this.x = canvas.width;
    this.y = groundLevel - this.height;
    this.speed = gameSpeed * 1.4; // Tốc độ = 7
    this.markedForDeletion = false;
    this.type = 'Type1';
    this.image = new Image();
...
class ObstacleType2 {
  constructor() {
    ...
    this.x = canvas.width;
    this.y = groundLevel - this.height;
    this.speed = gameSpeed * 1.1; // Tốc độ = 5.5
    this.markedForDeletion = false;
    this.type = 'Type2';
...
// --- CẬP NHẬT: Xử lý sự kiện bàn phím & NÚT BẤM ---
document.addEventListener('keydown', (event) => {
  // --- MỚI: Logic Bắt đầu Game ---
  ...
  if (event.code === 'Space') {
    event.preventDefault();
    if (!isGameOver && !isGameWon) player.jump();
  }
  ...
  if (event.code === 'KeyQ') {
    event.preventDefault();
    if (!isGameOver && !isGameWon && rage >= RAGE_MAX && isBossActive) {
      triggerKamehameha();
    }
  }
});

// --- MỚI: Xử lý sự kiện CHẠM (touch) cho nút bấm ---
// (Dùng 'touchstart' để phản hồi ngay lập tức)
if (jumpButton) {
  jumpButton.addEventListener('touchstart', (e) => {
    e.preventDefault(); // Ngăn double-tap zoom
    if (!isGameOver && !isGameWon && !isKameActive && !isShowingInstructions) {
      player.jump();
    }
    // Nếu đang ở màn hình hướng dẫn, bấm Nhảy cũng là Bắt đầu
    if (isShowingInstructions) {
      isShowingInstructions = false;
    }
  });
...
if (shootButton) {
  shootButton.addEventListener('touchstart', (e) => {
    ...
    // Nếu đang ở màn hình hướng dẫn, bấm Bắn cũng là Bắt đầu
    if (isShowingInstructions) {
      isShowingInstructions = false;
    }
    // Nếu game over/win, bấm Bắn cũng là Chơi lại
    if (isGameOver || isGameWon) {
      restartGame();
    }
  });
...
if (rageButton) {
  rageButton.addEventListener('touchstart', (e) => {
    e.preventDefault();
    if (!isGameOver && !isGameWon && rage >= RAGE_MAX && isBossActive && !isKameActive) {
      triggerKamehameha();
    }
  });
}
...
function drawRageBar() {
  ...
  if (rage >= RAGE_MAX && !isKameActive) {
    ...
    // --- MỚI: Hiện nút Nộ trên mobile ---
    if (rageButton) rageButton.style.display = 'block';
  }
}
...
function draw() {
  ...
  // --- MỚI: Vẽ màn hình Hướng dẫn (TRÊN CÙNG) ---
  if (isShowingInstructions) {
    ...
    ctx.font = '24px Arial';
    ctx.fillText("Nhấn [SPACE] để Nhảy", canvas.width / 2, canvas.height / 2 - 40);
    ctx.fillText("Nhấn [ENTER] để Bắn", canvas.width / 2, canvas.height / 2);
    ctx.fillText("Nhấn [Q] khi Nộ đầy", canvas.width / 2, canvas.height / 2 + 40);

    ctx.font = 'bold 26px Arial';
    ctx.fillStyle = '#FFFF00'; // Màu vàng
    ctx.fillText("Nhấn [SPACE] hoặc [ENTER] để Bắt đầu!", canvas.width / 2, canvas.height / 2 + 120);
  }
  ...
  if (isGameOver) {
    ...
    ctx.font = '20px Arial';
    ctx.fillText('Nhấn Enter để chơi lại', canvas.width / 2, canvas.height / 2 + 70);
  }
...
let isShaking = false;
let shakeTimer = 0;
let isShowingInstructions = true; // --- MỚI: Màn hình hướng dẫn ---
...
function restartGame() {
  ...
  isShowingInstructions = true; // --- MỚI: Hiển thị lại hướng dẫn ---
}

// --- CẬP NHẬT: Hàm kích hoạt Kamehameha (Audio) ---
function triggerKamehameha() {
  console.log("KAMEHAMEHA Sequence!");
  isKameActive = true;
  rage = 0;
...
// --- CẬP NHẬT: Xử lý sự kiện bàn phím ---
document.addEventListener('keydown', (event) => {
  ...
  if (isShowingInstructions) {
    ...
    return; // Dừng lại, không xử lý phím nào khác
  }
...
function drawRageBar() {
    ...
    if (rage >= RAGE_MAX && !isKameActive) {
        ...
        // --- CẬP NHẬT: Hiện nút Nộ trên mobile (nếu có) ---
        if (rageButton && rageButton.style.display === 'none') { // Chỉ hiện nếu đang ẩn
             rageButton.style.display = 'inline-block'; // hoặc 'block' tùy bố cục bạn muốn
        }
    } else {
        // --- MỚI: Ẩn nút Nộ khi không đủ nộ hoặc đang bắn chưởng ---
        if (rageButton && rageButton.style.display !== 'none') {
 ...
 */
function update() {
  // --- CẬP NHẬT: Thêm isShowingInstructions vào đây ---
  if (isGameOver || isGameWon || isShowingInstructions) return;
...
function draw() {
  ...
  if (isShowingInstructions) {
    ...
    ctx.fillText("Nhấn [ENTER] để Bắn", canvas.width / 2, canvas.height / 2);
    ctx.fillText("Nhấn [Q] khi Nộ đầy", canvas.width / 2, canvas.height / 2 + 40);
    ctx.fillText("Gặp BOSS khi 2000 điểm", canvas.width / 2, canvas.height / 2 + 80);
 ...
 */
function gameLoop() {
  // --- CẬP NHẬT: Logic Update mới ---
  // Chỉ update nếu game không over, không win, VÀ không hiện hướng dẫn
  if (!isGameOver && !isGameWon && !isShowingInstructions) {
    update();
  }
...
// Bắt đầu vòng lặp game!
>>>>>>> 5646b227172abc17c4b383a0fe35a133cc5f30b5
requestAnimationFrame(gameLoop);